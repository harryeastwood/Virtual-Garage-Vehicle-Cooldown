...
private["_serverTime", "_garageTimeout", "_storedTime", "_timeAfterStore", "_timeLeft", "_sessionID", "_parameters", "_nickname", "_playerObject", "_flagObject", "_territoryID", "_vehicleInfo", "_buildRights", "_storedVehicles", "_index"];
_sessionID = _this select 0;
_parameters = _this select 1;
_nickname = _parameters select 0;
_serverTime = round (serverTime);
// Garage Timeout in seconds - Set how long players have to wait before they can retrieve a vehicle that has just been stored in the virtual garage.
_garageTimeout = 300;
...
try
{
	...
	// Prevent Quick Storage after Pulling vehicle out THING
	_storedTime = (format["getStoredTime:%1", _vehicleInfo select 0] call ExileServer_system_database_query_selectSingle) select 0;
	
	_timeAfterStore = _serverTime - _storedTime;
	_timeLeft = _garageTimeout - _timeAfterStore;
	diag_log format ["VGTESTSTORE | Time After Pull: %1 | Time Left: %2", _timeAfterStore, _timeLeft];
	if (_timeAfterStore <= _garageTimeout) then
	{
		throw format ["You cannot retrieve this vehicle for another %1 seconds!", _timeLeft];
	};
	...
	format["retrieveVehicle:%1:%2", _serverTime, _vehicleInfo select 0] call ExileServer_system_database_query_fireAndForget;
