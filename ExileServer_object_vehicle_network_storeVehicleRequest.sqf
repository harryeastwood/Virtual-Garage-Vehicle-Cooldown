 ...
 *
 * Prevent Vehicle Storage above Certain Height override by [GADD]Monkeynutz.
 */

private["_serverTime", "_garageTimeout", "_retrieveTime", "_timeAfterPull", "_timeLeft", "_sessionID", "_parameters", "_netID", "_nickname", "_playerObject", "_vehicleObject", "_alphabet", "_forbiddenCharacter", "_flagObject", "_storedVehicles", "_level", "_maxNumberOfVehicles", "_vehiclePosition", "_items", "_magazines", "_weapons", "_containers", "_popTabs", "_groundHolder", "_popTabsObject", "_rearmVehs"];
...
_serverTime = round (serverTime);
// Garage Timeout in seconds - Set how long players have to wait before storeing a vehicle after it has been retrieved from the virtual garage.
_garageTimeout = 600;
_retrieveTime = (format["getRetrieveTime:%1", _vehicleObject getVariable ["ExileDatabaseID", -1]] call ExileServer_system_database_query_selectSingle) select 0;
try
{
	...
	// Prevent Quick Storage after Pulling vehicle out
	_timeAfterPull = _serverTime - _retrieveTime;
	_timeLeft = _garageTimeout - _timeAfterPull;
	if (_timeAfterPull <= _garageTimeout) then
	{
		throw format ["You cannot store this vehicle for another %1 seconds!", _timeLeft];
	};
	...
	format["storeVehicle:%1:%2:%3:%4", _flagObject getVariable ["ExileDatabaseID", -1], _nickname, _serverTime, _vehicleObject getVariable ["ExileDatabaseID", -1]] call ExileServer_system_database_query_fireAndForget;
